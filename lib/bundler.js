...
{
    // TODO: Don't cache middleware and check report (to get file list) in bundles to see if any sources have changed.
    ...
    if (!(this instanceof Middleware)) {
        ...
        {
            if (options.forceCompleteBuild !== true && options.rebuildChanges !== true) {
    ...
    this.options.forceCompleteBuild = this.options.forceCompleteBuild || false;
    this.options.reloadChanges = this.options.reloadChanges || true;
...
Middleware.prototype.handle = function(request, response)
{
    ...
    {
        var bundleLoader;
        if (self.options.bundleLoader && request.url === "") {
            bundleLoader = true;
        }
        
        request.url = "/" + PATH.basename(distPath) + (request.url || ".js");

        var rawBundlePath = PATH.dirname(distPath) + request.url,
            rawCode = FS.readFileSync(rawBundlePath).toString();
        
        if (typeof bundleLoader === "undefined") {

            // Look into the bundle report to see if the loader should be bundled.
            // TODO: Use bundle FS abstraction here.

            var partitionString = rawCode.match(/^\/\/\s*@sourcemint-bundle-partition-map\s*:\s*(.*?)\s*\n/);
            if (partitionString)
            {
                var partitionMap = JSON.parse(partitionString[1]);

                // NOTE: We read around the area provided by the partition map as it is not always accurate depending on the characters in the code.
                // TODO: Get the partition positions accurate.
                var report = JSON.parse(rawCode.substring(partitionMap.report[0] - 1000, partitionMap.report[1] + 1000).match(/\n\s*\/\/\s*@sourcemint-bundle-report\s*:\s*(.*?)\s*\n/)[1]);
                
                if (typeof report.bundleReport.bundleLoader !== "undefined")
                {
                    bundleLoader = report.bundleReport.bundleLoader;
                }
            }
        }

        if (bundleLoader === true)
        {
            // TODO: Only regenerate when changed.

            var bundledPath = rawBundlePath + ".bundled",
                loaderPath = require.resolve("sourcemint-loader-js/loader.js");
            ...
            var bundleCode = [
                ...
                '        ' + FS.readFileSync(loaderPath).toString().split('\n').join('\n        '),
                '    };',
                ...
                '        sourcemint.sandbox("' + ("http://" + request.headers.host + request.originalUrl) + '", function(sandbox) {',
                '            sandbox.main();',
                '        }, {',
            ...
            FS.writeFileSync(bundledPath, bundleCode.join("\n"));
            request.url += ".bundled";
        }
